        ...
        for row in range(self.height):
            for col in range(self.width):
                ...
                if row == 0:
                    ...
                    screen.blit(number, ((self.margin + WIDTH) *
                                col + self.margin + 8, self.margin - 20))
                if col == 0:
                    number = self.font.render(str(row+1), True, BLACK)
                    screen.blit(number, (self.margin - 20,
                                (self.margin + HEIGHT) * row + self.margin + 8))
...
class Field:
    ...
    def fill(self, row, col, color):
        pygame.draw.rect(screen, color, [(self.grid.margin + WIDTH) * col + self.grid.margin,
                                         (self.grid.margin + HEIGHT) *
                                         row + self.grid.margin,
                                         WIDTH, HEIGHT])
        self.grid.grid[row][col] = 1

    def invFill(self, row, col, color):
        pygame.draw.rect(screen, color, [(self.grid.margin + WIDTH) * col + self.grid.margin,
                                         (self.grid.margin + HEIGHT) *
                                         row + self.grid.margin,
                                         WIDTH, HEIGHT])
    ...
    def isfilled(self, row, col):
        ...
        if (row < 0 or row > 19 or col < 0 or col > 19):
            ...
            return self.grid.grid[row][col] == 1
        # return row < 0 or row > 19 or col < 0 or col > 19 or self.grid.grid[row-1][col-1] == 1
...
class Labyrinth:
    ...
    def goLeft(self):
        ...
        self.field.invFill(self.current[0], self.current[1], RED)
...
while not done:
    ...
    field.invFill(l.start[0], l.start[1], BLUE)
    field.invFill(l.end[0], l.end[1], GREEN)
